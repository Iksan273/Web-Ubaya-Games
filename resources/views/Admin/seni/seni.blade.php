    ...
    <div class="card">
        <div class="card-body">
            <div class="container-fluid">
                <h2 class="text-center">Data Cabang Seni</h2><br>
                        ...
                        <tbody>
                            @foreach ($seni as $data)
                                <tr>
                                    ...
                                    <td>
                                        @php
                                            $cabangArray = explode(', ', $data->cabang);
                                        @endphp

                                        @foreach ($cabangArray as $cabang)
                                            <span class="badge badge-primary" >{{ $cabang }}</span>
                                        @endforeach
                                    </td>
                                    ...
                                    <td>
                                        <a href="{{ asset('seni/files/' . $data->file) }}" style="color: white; text-decoration: none;">
                                            <button type="button" class="btn btn-success text-white"> View </button>
                                        </a>
                                        &nbsp; <!-- Menambahkan spasi antara tombol -->
                                        <a href="{{ asset('seni/files/' . $data->file) }}" download="{{ $data->file }}"
                                           >
                                    ...
                                    <td>
                                        <a href="{{ route('admin.editSeni',['id'=>$data->id]) }}"><button class="btn btn-primary btn-sm"
                            ...
                            <div class="modal fade" id="deleteModal{{ $data->id }}" tabindex="-1" aria-labelledby="deleteModalLabel"
                                ...
                                <div class="modal-dialog">
                                    <div class="modal-content">
                                        <form id="deleteForm" action="{{ route('admin.deleteSeni',['id' => $data->id]) }}" method="POST">
